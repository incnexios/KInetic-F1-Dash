...
ws.on('message', (data) => {
    ...
    if (j.Position) {
        ...
        if (Array.isArray(j.Position)) { console.log('Wait array?!'); } else {
             console.log('Position keys:', Object.keys(j.Position));
             console.log('Position inner type:', Array.isArray(j.Position.Position) ? 'array' : typeof j.Position.Position);
